          ...
          <div className='flex relative items-center gap-[70px] xl:gap-10 leading-6 h-full'>
            {
              categories.map(item => {
                return(
                  <NavLink key={item.id} to={`/${item.name.toLowerCase()}`} className={(navData) => (navData.isActive ? "text-[#7399FF] border-spacing-2 border-[#7399FF] border-t-2 h-full flex items-center pb-[2px] stroke-[#7399FF]" : 'h-full flex items-center hover:text-[#7399FF] transition-all')}>
                  <p>{item.name}</p>                                                                                             
                  </NavLink>
                )
              })
