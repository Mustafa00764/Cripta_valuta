...
const Header = () => {
    ...
    try {
      const response = await api.get('/auth/telegram/callback', {
        params: {
          ...
          photo_url: user.photo_url ? user.photo_url : LC_logo,
        },
      });
  ...
  const photoUrl = (styles) => {
    if (!user) {
      return;
    }
    if (user.photo_url.startsWith("https://legitcommunity.uz")) {
      return <img src={user ? user.photo_url : ""} alt="" className={styles} crossOrigin="anonymous" />
    } else {
      return <img src={user ? user.photo_url : ""} alt="" className={styles} />
    }
  }
        ...
        <div className={`absolute bg-[#2F2F2F] w-[300px] top-[136px] transition-all duration-[50] lg:top-[100px] shadow-lg md:top-[80px] ms:top-[64px] rounded-b-[15px] right-5 z-20 ${dmenu ? " h-auto opacity-[1]" : "opacity-0 h-0 overflow-hidden"}`} onMouseOver={() => setDmenu(true)} onMouseOut={() => setDmenu(false)}>
          ...
          <div className=" text-[14px]">
            <div className=" px-6 h-[100%] text-[#fff] flex flex-col pb-1">
              <NavLink to={`/profile/${user ? user.id : ""}`} onClick={() => setMenu(false)} className={(navData) => (navData.isActive ? ` font-extrabold` : " ")}>
