...
const DAModel = ({handleArticlesList}) => {
  ...
  },[article,deleteArticleModel])

  return (
    <div className={deleteArticleModel?'w-full h-full absolute top-0 left-0 flex justify-center transition-all bg-[rgba(0,0,0,.5)] items-center z-[25] opacity-[1]':'w-full h-full absolute top-0 left-0 flex justify-center transition-all bg-[rgba(0,0,0,.5)] items-center -z-10 opacity-0'}>
    <div className='max-w-[767px] w-full flex flex-col gap-4 h-auto p-6 bg-bgMode rounded-[15px] '>
      ...
      <p className='text-[32px] leading-[48px] font-semibold text-center'>Вы уверены, что хотите удалить эту статью?</p>
      <div className='w-full flex justify-center gap-6 items-center'>
        ...
        <button className='mt-4 max-w-[250px] w-full text-[24px] h-[50px] rounded-[8px] bg-blue-500 text-white' onClick={()=>setDeleteArticleModel(false)}>
          Cancel
        </button>
      </div>
    </div>
